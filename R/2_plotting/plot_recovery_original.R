...
fit_orl <- readRDS("Data/Recovered/orl_originalIGT_N48_recovered.rds") # 
# PVL Delta
pvl_AhnReal <- read.table("Data/Simulated/pvl_delta_simulated_indPars.txt", header = T)
fit_pvl <- readRDS("Data/Recovered/pvl_delta_originalIGT_N48_recovered.rds") # 
# VPP
vpp_AhnReal <- read.table("Data/Simulated/vpp_simulated_indPars.txt", header = T)
fit_vpp <- readRDS("Data/Recovered/vpp_originalIGT_N48_recovered.rds") # 
...
h2 <- ggplot() +
  ...
  geom_point(data = indPars_vpp, 
             ...
             size = 2, alpha = 0.4, color = I("#ff7f00")) + 
  geom_point(data = indPars_vpp, 
             aes(x = scale(epN_real), y = scale_rel(epN,epN_real)), 
             size = 2, alpha = 0.4, color = I("#ffff33")) + 
  geom_point(data = indPars_vpp, 
             aes(x = scale(K_real), y = scale_rel(K,K_real)), 
             size = 2, alpha = 0.4, color = I("#a65628")) + 
  geom_point(data = indPars_vpp, 
             aes(x = scale(w_real), y = scale_rel(w,w_real)), 
             size = 2, alpha = 0.4, color = I("#f781bf")) +
  coord_cartesian(xlim = c(-4,4), ylim = c(-4,4)) +
  geom_abline(intercept = 0, slope = 1) + 
  geom_abline(intercept = 1, slope = 1, lty = 2, color = "black") + 
  geom_abline(intercept = 2, slope = 1, lty = 3, color = "black") + 
...
ggsave(filename="~/Desktop/recover_orig.pdf", plot = all_plots, units = "in",
