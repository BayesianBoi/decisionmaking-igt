...
hier_PVL_sim <- function(payoff_struct, nsubs, ntrials,
    ...
    for (s in 1:nsubs) {
        ...
        # Bounded to [0,1] to match pvl_delta_v2.txt
        ...
        for (t in 1:ntrials[s]) {
            ...
            # Using abs() + 0.0001 to prevent 0^fractional issues (matches model v2)
            if (outcome >= 0) {
                curUtil <- (abs(outcome) + 0.0001)^A
            } else {
                curUtil <- -w * (abs(outcome) + 0.0001)^A
            }
