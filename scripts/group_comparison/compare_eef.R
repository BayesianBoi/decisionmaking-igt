...
# compare_eef.R
# -------------
# Joint hierarchical EEF model for comparing two groups.
#
...
# Example: Rscript compare_eef.R HC Amph
...
if (length(args) != 2) {
    stop("Must specify TWO groups. Example: HC Amph")
}
...
prep_group_data <- function(data, study_label) {
    ...
    for (s in seq_len(nsubs)) {
        ...
        # Scale by dividing by 100 (numerical stability)
        X_sub <- X_raw / 100
        length(X_sub) <- ntrials_max
        X_all[s, ] <- X_sub
    }
...
model_file <- "models/eef_compare.txt"
