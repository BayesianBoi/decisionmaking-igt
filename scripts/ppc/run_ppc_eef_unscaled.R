#!/usr/bin/env Rscript
# =============================================================================
# INDIVIDUAL PPC - EEF Model - UNSCALED VERSION
# =============================================================================
# Fits EEF model individually for each subject and computes PPC
# This version uses UNSCALED outcomes (original monetary values)
# Usage: Rscript scripts/ppc/run_ppc_eef_unscaled.R <group>
# Example: Rscript scripts/ppc/run_ppc_eef_unscaled.R HC
# =============================================================================
...
if (length(args) < 1) {
    stop("Usage: Rscript run_ppc_eef_unscaled.R <group>\nGroup: HC, Amph, Hero")
}
...
cat("\n================================================\n")
cat("INDIVIDUAL PPC: EEF (UNSCALED) -", target_group, "\n")
cat("================================================\n")
...
process_subject <- function(s) {
    ...
    # Prepare data with separate Gain and Loss - UNSCALED
    x <- subj_df$choice
    Gain <- subj_df$gain # NO /100 scaling
    Loss <- subj_df$loss # NO /100 scaling
...
results_path <- file.path(output_dir, paste0("ppc_individual_eef_unscaled_", target_group, ".rds"))
...
p <- ggplot(pred_df, aes(x = subject, y = accuracy)) +
    ...
    ylim(0, 0.8) +
    labs(
        title = paste("Individual PPC: EEF (UNSCALED) -", target_group),
...
plot_path <- file.path(output_dir, paste0("ppc_individual_eef_unscaled_", target_group, ".png"))
