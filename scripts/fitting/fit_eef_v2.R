#!/usr/bin/env Rscript
# =============================================================================
# FIT EEF V2 MODEL (JAGS)
# =============================================================================
# Fits the Exploitation-Exploration with Forgetting model (Yang et al. 2025)
# This version matches the original paper exactly:
#   - V(t) = Gain(t)^theta - Loss(t)^theta (separate gain/loss inputs)
#   - Exploitation chosen: (1-lambda)*Exploit + V(t)  [no lambda on V(t)]
#   - Exploration unchosen: lambda*Explore + (1-lambda)*phi
#
# Usage: Rscript fit_eef_v2.R [HC|Amph|Hero]
# =============================================================================
...
for (s in 1:nsubs) {
    ...
    # Separate gain and loss (loss is stored as negative, we keep it that way)
...
# JAGS Fitting
output_dir <- "outputs/eef_v2"
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

model_file <- "models/eef_v2.txt"
...
cat("Fitting hierarchical EEF v2 model for group:", target_group, "...\n")
...
# Save Results
save_path <- file.path(output_dir, paste0("eef_v2_fit_", target_group, ".rds"))
