...
# check_convergence.R
# -------------------
# Checks MCMC convergence diagnostics for a fitted model.
#
# We look at two main things:
#   1. Rhat (Gelman-Rubin statistic) - should be below 1.1, ideally below 1.05
#      Values above 1.1 mean the chains haven't converged to the same distribution.
#   2. n.eff (effective sample size) - should be above 100, ideally above 400
#      Low values mean high autocorrelation, so the samples aren't independent.
#
# The p parameter (trial-by-trial probabilities) is excluded from diagnostics
# because it's massive and not directly interpreted.
#
# Usage: Rscript check_convergence.R <model> <group>
# Example: Rscript check_convergence.R orl HC

args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 2) {
    stop("Usage: Rscript check_convergence.R <model> <group>")
}
...
# Get parameter names, excluding the p array
...
cat("Target: Rhat < 1.1 (ideal < 1.05)\n\n")
...
cat("Target: n.eff > 100 (ideal > 400)\n\n")
