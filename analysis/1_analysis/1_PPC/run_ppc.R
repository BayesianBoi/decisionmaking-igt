...
if (model_name == "eef") {
    fit_path <- file.path(
        "results from recovery/parameter estimation results/eef",
        paste0("eef_fit_", target_group, ".rds")
    )
} else if (model_name == "orl") {
    fit_path <- file.path(
        "new parameter estimation results",
        paste0("orl_fit_", target_group, ".rds")
    )
} else {
